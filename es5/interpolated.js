...
var interpolated = function interpolated() {
	...
	var stage = isSvg ? dom.svg("svg", { width: 1, height: 1 }) : dom.canvas(1);
	var initialSettings = { background: { type: "Boolean", label: "Background", cur: true } };
	...
	var init = function init(options) {
		...
		var layer1Rotation = layer0Rotation + (isRotated ? r.getNumber(-30, 30) : 0);
		var layer1Offset = isRotated ? { x: 0, y: 0 } : { x: r.getNumber(-50, 50) / 1000, y: r.getNumber(-50, 50) / 1000 };

		var sameColourMod = r.getNumber(0, 1);
		var bgColour = c.getRandomColour();
		var drawLayer0 = r.getNumber(0, 1) > 0.3;
		var drawLayer1 = drawLayer0 ? r.getNumber(0, 1) > 0.5 : true;
		...
		if (isSvg) {
			dom.setAttributes(stage, { width: size, height: size, viewBox: "0 0 " + size + " " + size });
			inner = dom.svg("g", { id: "curves" });
			bg = dom.svg("rect", { x: 0, y: 0, width: size, height: size, style: "fill:" + bgColour });
			layer0Mask = dom.svg("mask", { id: "zeroMask" });
			layer0Show = dom.svg("rect", { x: -size, y: -size, width: size * 3, height: size * 3, style: "fill:" + maskColourShow });
			layer1Mask = dom.svg("mask", { id: "oneMask" });
			layer1Show = dom.svg("rect", { x: -size, y: -size, width: size * 3, height: size * 3, style: "fill:" + maskColourHide });
			...
			layerCurves.ctx.translate((bmpSize - size) / 2, (bmpSize - size) / 2);
			layer0Bmp = dom.canvas(bmpSize);
			layer1Bmp = dom.canvas(bmpSize);
		}

		var oscillators = Array(points * 4).fill().map(function () {
			return { o: r.getNumber(-Math.PI, Math.PI), s: r.getNumber(0.001, 0.004) * 5 };
		});
		...
		var drawCurve = function drawCurve(li, _ref2) {
			...
			var arrPoints = Array(points).fill().map(function (_, pi) {
			...
			Array(detail).fill().forEach(function (_, i) {
				var x = -offScreen + i / detail * (1 + 2 * offScreen);
				var y = lag.valueOf(x);
			...
			return isSvg ? dom.svg("path", { d: path.join(" "), fill: "transparent", stroke: colour, "stroke-width": strokeWidth }) : path;
		};

		var basePoints = Array(lines).fill().map(function (_, li) {
			return Array(points).fill().map(function (_, pi) {
				return getBasePoint(li, pi);
			});
		});

		var lastColour = c.getRandomColour();
		Array(lines).fill().forEach(function (_, li) {
			...
			lastColour = colour;
			var strokeWidth = r.getNumber(1 / 1000, maxStrokeWidth) * size;
			var curve = drawCurve(li, { colour: colour, strokeWidth: strokeWidth });
		...
		var polyPath = function polyPath(_ref3) {
			var radius = _ref3.radius,
			    sides = _ref3.sides;
			return Array(sides).fill().map(function (_, j) {
		...
		if (isSvg) {
			...
			var drawPolygonSvg = function drawPolygonSvg(_ref4) {
				...
				var path = polyPath({ radius: radius, sides: sides }).map(function (_ref5, j) {
					var x = _ref5.x,
					    y = _ref5.y;
					return (j === 0 ? "M" : "L") + x + "," + y;
				}).concat("Z");
			...
			var shapeItSvg = function shapeItSvg(_ref7) {
				...
				var shape = isPoly ? drawPolygonSvg({ sides: sides, radius: radius }) : drawCircleSvg({ radius: radius });
				shape.setAttribute("fill", overlapStroked ? "transparent" : masker);
				shape.setAttribute("stroke", masker);
				shape.setAttribute("stroke-width", 100 / 1000 * size);
				shape.setAttribute("stroke-linejoin", "round");
				shape.setAttribute("transform", "translate(" + (half - x) * size + "," + (half - y) * size + ") rotate(" + rotation + ")");
				return shape;
			};

			if (drawLayer0) {
				var layer0Transform = "translate(" + half * size + "," + half * size + ") scale(1.2) rotate(" + layer0Rotation + ") translate(" + -half * size + "," + -half * size + ")";
				var layer0Use = dom.svg("use", { transform: layer0Transform, href: "#curves", mask: "url(#zeroMask)" });
				stage.appendChild(layer0Use);
				var layer0Shape = shapeItSvg({ masker: maskColourHide, rotation: startAngle - layer0Rotation, x: 0, y: 0 });
				layer0Mask.appendChild(layer0Shape);
			}

			if (drawLayer1) {
				var layer1Shape = shapeItSvg({ masker: maskColourShow, rotation: startAngle - layer1Rotation, x: layer1Offset.x, y: layer1Offset.y });
				layer1Mask.appendChild(layer1Shape);
				var layer1Transform = "translate(" + half * size + "," + half * size + ")\n\t\t\t\t\tscale(1.2) rotate(" + layer1Rotation + ")\n\t\t\t\t\ttranslate(" + (-half + layer1Offset.x) * size + "," + (-half + layer1Offset.y) * size + ")";
				var layer1Use = dom.svg("use", { transform: layer1Transform, href: "#curves", mask: "url(#oneMask)" });
			...
			var drawPolygonBmp = function drawPolygonBmp(_ref8) {
				...
				polyPath({ radius: radius, sides: sides }).map(function (_ref9, j) {
					var x = _ref9.x,
					    y = _ref9.y;
			...
			var shapeItBmp = function shapeItBmp(canvas, masker, rotation) {
				canvas.ctx.save();
				canvas.ctx.translate(half * size, half * size);
				canvas.ctx.scale(1.2, 1.2);
				canvas.ctx.rotate(rotation / 180 * Math.PI);
				isPoly ? drawPolygonBmp({ canvas: canvas, sides: sides, radius: radius }) : drawCircleBmp({ canvas: canvas, radius: radius });
				canvas.ctx.fillStyle = overlapStroked ? "transparent" : masker == maskColourHide ? bgColour : masker;
				canvas.ctx.fill();
			...
			if (drawLayer0) {
				...
				layer0Bmp.ctx.translate(-half * bmpSize, -half * bmpSize);
				...
				shapeItBmp(temp0, maskColourHide, startAngle - layer0Rotation);
				layer0Bmp.ctx.globalCompositeOperation = "destination-out";
			...
			if (drawLayer1) {
				...
				layer1Bmp.ctx.translate(-half * bmpSize, -half * bmpSize);
				layer1Bmp.ctx.translate(layer1Offset.x * size, layer1Offset.y * size);
				layer1Bmp.ctx.drawImage(layerCurves.canvas, 0, 0);
				layer1Bmp.ctx.restore();

				// layer 1 shape
				var temp1 = dom.canvas(bmpSize);
				shapeItBmp(temp1, maskColourShow, startAngle - layer0Rotation);
				layer1Bmp.ctx.globalCompositeOperation = "destination-in";
