...
var render = function render(frame) {
  ...
  var scale =
    Math.sin((frame / frames) * Math.PI * 2 - Math.PI / 2) +
    1.98; // 2 is perfect here, but no to harmony.
  //scale *= 0.1;
  unit = 51; //(Math.sin(time * 0.0002) + 2) * 30;
  var angle =
    (Math.floor(frame / frames) * Math.PI * 2) / 3 / 4; //Math.sin(time * 0.0001 * Math.PI);
  ctx.fillStyle = "#000";
  ...
  while (gx++ < gridX) {
    ...
    while (gy++ < gridY) {
      var x =
          centre + (gx - gridX / 2) * cosThirty * unit * scale,
        y =
          centre +
          ((gx % 2) * 3 + (gy - gridY / 2) * 6) *
            sinThirty *
