...
var synth_ambient = function synth_ambient(Tone) {
  ...
  function init() {
    ...
    dom.on(loadNext, ["click"], function () {
      window.location =
        "?synth_ambient," + Math.round(Math.random() * 1e10);
    });
    ...
    var crusher = new Tone.BitCrusher(
      rand.getInteger(2, 6),
    ).toMaster();
    ...
    var hihatClosed = connectGenerator(
      new Tone.NoiseSynth({
        volume: -9,
        envelope: {
          attack: 0.001,
    ...
    var hihatOpen = connectGenerator(
      new Tone.NoiseSynth({
        volume: -10,
        filter: {
          Q: 1,
    ...
    var synthPoly = connectGenerator(
    ...
    Tone.Transport.scheduleRepeat(function (time) {
      var note =
        notesArpeggio[
          noteArpeggioCurrent % notesArpeggio.length
        ];
      if (noteArpeggioCurrent < notesArpeggio.length)
        synthArpeggio.triggerAttackRelease(note, "16n", time);
      noteArpeggioCurrent++;
