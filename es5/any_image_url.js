...
var any_image_url = function any_image_url() {
	return function () {
		...
		function render() {
			...
			var ok = allowed[r.getSeed()];
			if (ok && ok.image) {
				renderBMPFromURL(ok.image, ok.scale);
			} else {
				con.warn("Cannot find image:", r.getSeed());
				console.log(ok);
				progress("render:complete", null);
			}
		...
		function renderBMPFromURL(url, scale) {
			...
			if (isNode) {
				...
				var img = new Image();
				img.onload = function () {
					// con.log("renderBMPFromURL img.onload");
					drawToContext(img);
				};
				img.onerror = function (err) {
					con.log("renderBMPFromURL img.onerror error", err);
				};
				img.src = url;
			}
		}
		...
		function loadImageURL(url, fulfill, reject) {
			...
			protocol.get(url, function (res) {
				...
				res.on("end", function () {
					...
					// con.log("loadImageURL on.end", length);
					fulfill(loaded);
				});
