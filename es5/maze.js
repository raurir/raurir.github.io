(function () {
	...
	logShape = (function (_this) {
		return function () {
			...
			for (
				y = _i = 0;
				0 <= yhigh ? _i < yhigh : _i > yhigh;
				y = 0 <= yhigh ? ++_i : --_i
			) {
				for (
					x = _j = 0;
					0 <= xwide ? _j < xwide : _j > xwide;
					x = 0 <= xwide ? ++_j : --_j
				) {
					s += field[y][x];
				}
   ...
   */
	...
	for (
		y = _k = 0;
		0 <= yhigh ? _k < yhigh : _k > yhigh;
		y = 0 <= yhigh ? ++_k : --_k
	) {
		for (
			x = _l = 0;
			0 <= xwide ? _l < xwide : _l > xwide;
			x = 0 <= xwide ? ++_l : --_l
		) {
			if (
				x === 0 ||
		...
		again = (function (_this) {
			return function () {
				...
				if (keepDrawing()) {
					...
					for (
						y = _o = 0;
						0 <= yhigh ? _o < yhigh : _o > yhigh;
						y = 0 <= yhigh ? ++_o : --_o
					) {
						for (
							x = _p = 0;
							0 <= xwide ? _p < xwide : _p > xwide;
							x = 0 <= xwide ? ++_p : --_p
						) {
							f = field[y][x];
							if (f === "?" || f === ",") {
