...
var colours = function colours(rand, version) {
  ...
  function getRandomPalette(warning) {
    if (warning)
      console.warn("Ensure you call getRandomPalette!");
  ...
  function setPaletteRange(range) {
    if (range > currentPalette.length)
      return console.warn(
        "setPaletteRange - current palette has less than",
        range,
        "colours!",
      );
  ...
  function convertHex3To6(hex) {
    ...
    throw new Error(
      "colours.convertHex3To6 - invalid colour type:" +
        hex +
        " - expected three digit hex value!",
    );
  }

  function mutateChannel(channel, amount, direction) {
    var mutation = Math.round(
      channel + (random() - 0.5) * amount,
    );
    mutation =
      mutation > 255 ? 255 : mutation <= 0 ? 0 : mutation;
    return mutation;
  }
  ...
  function mutateColour(colour, amount) {
    ...
    throw new Error(
      "colours.mutateColour - invalid colour type:" + colour,
    );
  ...
  function showColours() {
    ...
    for (var i = 0; i < palettes.length; i++) {
      ...
      var testButton = dom.element("button", {
        innerHTML:
          darkest.colour + " (" + ~~darkest.brightness + ")",
        style: {
          color: "white",
  ...
  return {
    ...
    getPalleteIndex: function getPalleteIndex() {
      return paletteIndex;
    },
