...
var bezier_flow = function bezier_flow() {
  var r = rand.instance();
  var c = colours.instance(r);
  console.log("bezier_flow setting seed.");
  ...
  function init(options) {
    ...
    for (var l = 0; l < lines; l++) {
      lineStyles[l] = {
        ...
        lineWidth: constantBaseLine
          ? fixedConstantBaseLine
          : baseLineWidth(),
      };
    }
  ...
  */
    ...
    for (var p = 0; p < sections; p++) {
      ...
      var a =
        baseAngle +
        (p / sections +
          r.getNumber(-angleVariance, angleVariance)) *
          Math.PI *
          2;
  ...
  function createPoint(origin) {
    ...
    for (var i = 0; i < lines; i++) {
      ...
      total +=
        gap *
        (exponential ? Math.pow(2, 1 + i * 0.2) * 0.1 : 1);
    }
  ...
  function render() {
    ...
    for (var j = 0; j < settings.renderlimit.cur; j++) {
      ...
      for (var i = 0, il = points.length; i < il; i++) {
        ...
        ctx.lineWidth =
          lineStyles[j].lineWidth *
          (scalePerLine ? (j + 1) * 0.1 : 1);
