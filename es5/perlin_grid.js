...
var perlin_grid = function perlin_grid(noise) {
	...
	function cube(options) {
		...
		var shaderRed = createMaterial(
			checker[0][0],
			checker[0][1],
			colour,
			distance,
		);
		var shaderYellow = createMaterial(
			checker[1][0],
			checker[1][1],
			colour,
			distance,
		);
		var shaderGreen = createMaterial(
			checker[2][0],
			checker[2][1],
			colour,
			distance,
		);
		var shaderBlue = createMaterial(
			checker[3][0],
			checker[3][1],
			colour,
			distance,
		);
		var shaderCyan = createMaterial(
			checker[4][0],
			checker[4][1],
			colour,
			distance,
		);
		var shaderMagenta = createMaterial(
			checker[5][0],
			checker[5][1],
			colour,
			distance,
	...
	var draw = function draw(x, z, distanceFromCentre) {
		...
		horizontalEdgeF.position.set(
			0,
			(size.height + edgeSize) / 2,
			-distance,
		);
	...
	function render(time) {
		...
		function renderBox(holder, value, x, z, above) {
			...
			var px =
				(x - gridUnits / 2 + 0.5) * size.width * breathDistance;
			var py = ((scale * size.height) / 2) * above;
			var pz =
				(z - gridUnits / 2 + 0.5) * size.depth * breathDistance;
		...
		for (var z = 0; z < gridUnits; z++) {
			...
			for (var x = 0; x < gridUnits; x++) {
				...
				var valueAbove =
					(noise.perlin3(x / gridUnits, z / gridUnits, seed) +
						1) /
					2;
				var valueBelow =
					(noise.perlin3(
						x / gridUnits,
						z + (gridUnits * gridUnits) / gridUnits,
						seed,
					) +
						1) /
					2;
		...
		var rotY = camPos.x * 4;
		camera.position.set(
			Math.sin(rotY) * camDistance,
			camPos.y,
			Math.cos(rotY) * camDistance,
		);
	...
	function animate(time) {
		...
		if (Math.random() > 0.99) {
			var holder =
				Math.random() > 0.5 ? holderAbove : holderBelow;
