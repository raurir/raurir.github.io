...
var exps = function exps(experimentsDetails) {
  ...
  return function () {
    ...
    document.documentElement.style.setProperty(
      "--colour-accent",
      "hsl(" + randomHue + ", 100%, 60%)",
    );
    document.documentElement.style.setProperty(
      "--colour-accent-20",
      "hsla(" + randomHue + ", 100%, 60%, 0.2)",
    );
  ...
  */
    ...
    var panelButtonClose = dom.button("X", {
      className: "exps-button",
    });
    ...
    var showButtons = function showButtons() {
      ...
      for (var e in experiments) {
        ...
        dom.on(button, ["click"], function (event) {
          window.location =
            "?" + event.target.key + (viewSource ? "&src" : "");
        });
    ...
    var showSource = function showSource(srcFile, language) {
      ...
      fetch(sourcePath)
        ...
        .catch(function (error) {
          ...
          panelInfoDetails.innerHTML =
            "\n<h4>EXPGFX:SRC:ERROR</h4>\n<p>Could not load source code: " +
            error.message +
            "</p>\n<p><a href='#' class='back-to-info-link'>\u2190 Back to Info</a></p>\n";
        })
        .finally(function () {
          var backLink = panelInfoDetails.querySelector(
            ".back-to-info-link",
          );
    ...
    */
        ...
        if (viewSource) {
          console.log(
            "`src` in url: Experimental graphics in SRC mode...",
          );
        }
        if (key === "src") {
        ...
        if (setSize.length === 1) {
          var _setSize$0$split = setSize[0].split("="),
            _setSize$0$split2 = _slicedToArray(
              _setSize$0$split,
              2,
            ),
        ...
        if (setFormat.length === 1) {
          var _setFormat$0$split = setFormat[0].split("="),
            _setFormat$0$split2 = _slicedToArray(
              _setFormat$0$split,
              2,
            ),
        ...
        var observer = new MutationObserver(function (
          ...
          mutations.forEach(function (mutation) {
            if (
              mutation.target.className === "exps-info-details"
            )
    ...
    var resize = function resize() {
      ...
      if (currentExperiment && currentExperiment.resize)
   ...
   */
    ...
    var experimentLoaded = function experimentLoaded(exp) {
      currentExperiment =
        typeof exp === "function" ? exp(format === "svg") : exp;
      if (currentExperiment.stage) {
        holder.appendChild(currentExperiment.stage);
      } else {
        console.warn(
          "experimentLoaded, but no stage:",
          currentExperiment.stage,
        );
      }
