...
define("lego_stack", [
	...
	function go() {
		...
		function createBlock() {
			...
			var knob = new CANNON.Cylinder(
				kRadius,
				kRadius,
				kLength,
				12,
			);
			var roof = new CANNON.Box(
				new CANNON.Vec3(width, length, wallThickness),
			);
			var bodyT = new CANNON.Box(
				new CANNON.Vec3(width, wallThickness, height),
			);
			var bodyR = new CANNON.Box(
				new CANNON.Vec3(wallThickness, length, height),
			);
			var bodyB = new CANNON.Box(
				new CANNON.Vec3(width, wallThickness, height),
			);
			var bodyL = new CANNON.Box(
				new CANNON.Vec3(wallThickness, length, height),
			);
			...
			block.addShape(
				bodyT,
				new CANNON.Vec3(0, length - wallThickness, 0),
			);
			block.addShape(
				bodyR,
				new CANNON.Vec3(width - wallThickness, 0, 0),
			);
			block.addShape(
				bodyB,
				new CANNON.Vec3(0, -length + wallThickness, 0),
			...
			var colour = [0xdd0000, 0x004400, 0xffbb00, 0x2222ee][
				rndI.getInteger(0, 3)
			];
		...
		function update(time) {
			...
			if (
				blocks.length < 100 &&
				blocks.length < Math.floor(time / 1000)
			) {
