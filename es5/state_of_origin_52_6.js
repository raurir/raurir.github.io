...
var state_of_origin_52_6 = function state_of_origin_52_6() {
	return function () {
		var progress;
		var QLD = "QLD",
			NSW = "NSW";
		...
		function initState(state, score, x, y, maxGlyphs) {
			...
			function pointInShape(point) {
				...
				var pixels = testCanvas.ctx.getImageData(
					xs,
					ys,
					width,
					height,
			...
			function getScaleFromCount(c) {
				...
				var sc =
					0.75 +
					(rand.random() * 4 * (0.001 + countMax - c)) /
						countMax;
				// con.log(c, sc);
				return sc;
			}
			...
			progressCanvas.ctx.globalCompositeOperation =
				"destination-out";
		...
		function init(options) {
			progress =
				options.progress ||
				function () {
					con.log("state_of_origin_52_6 - no progress defined");
				};
		...
		function loop() {
			...
			if (complete.NSW && complete.QLD) {
				...
				progress(
					"render:progress",
					(progressions.NSW + progressions.QLD) / 2,
				);
				setTimeout(loop, 1);
			}
		}
