	...
	function init() {
		...
		function addHex(x, y, z, isEdge) {
			...
			hex.move = function () {
				...
				this.kindadecided -=
					(this.kindadecided - this.undecided) * 0.1;
		...
		function pickOne() {
			return hexsTargetable[
				~~(Math.random() * hexsTargetable.length)
			];
		}
		...
		function render(t) {
			...
			camera.position.x -=
				(camera.position.x - target.position.x) * 0.02;
			camera.position.y -=
				(camera.position.y - target.position.y) * 0.02;
